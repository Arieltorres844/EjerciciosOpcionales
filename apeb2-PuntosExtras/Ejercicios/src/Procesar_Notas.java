...
public class Procesar_Notas {
    ...
    public static void persistirResultados(String nombreArchivoOut, String notasIn [][], String notasOut [][]){
        try {
            ...
            int [] conteo = contarApro(notasOut);
            fout.format("\nAprobado;%d\n", conteo[0]);
            fout.format("Reprobado;%d\n", conteo[1]);
            debajoMedia(fout, notasIn, notasOut);
            fout.close();
        } catch (Exception e) {
        }
    }
    ...
    public static int [] contarApro(String datosOut [][]){
        int cp = 0;
        int cn = 0;
        for (int i = 0; i < datosOut.length; i++) {
            if (datosOut[i][2].equals("Aprobado")) {
                cp++;
            } else {
                cn++;
            }
            }
        return new int[]{cp, cn};
        }
    public static double promedioGeneral(String datosOut[][]) {
        double suma = 0;
        double promedio = 0;
        for (int i = 0; i < datosOut.length; i++) {
            suma += Double.valueOf(datosOut[i][0]);
        }
        return promedio = suma / datosOut.length;
    }

    public static void debajoMedia(Formatter fout,String datosNotas[][], String datosOut[][]) {
        double media = promedioGeneral(datosOut);
        fout.format("Promedio General;%.2f\n" ,media);
        fout.format("\nDebajo de la media\n");
        for (int i = 0; i < datosOut.length; i++) {
            double promedio = Double.valueOf(datosOut[i][0]);
            if (promedio < media) {
                fout.format("%s;%.2f\n" ,datosNotas[i][0], promedio);
            }
        }
    }
}
